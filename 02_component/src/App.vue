<template>
  <div>
    ...
    <ul>
      ...
      <CheckboxItem v-for="student in students" :key="student.no" 
          :student="student" @item-change="itemChange"/>
    </ul>
    <h3>2. 슬롯(slot)</h3>
    <p>
      부모 컨포넌트에서 자식 컴포넌트로 탬플릿을 전달하는 방법을 제공한다.
    </p>
    <ul>
      <!-- 슬롯 데스트 -->
      <!-- 
  ...
  export default {
    ...
    methods: {
      ...
      slotChange(data) {
        ...
        const slotItem = this.slotItems.find(slotItems => slotItems.id === data.id);
